...
class Evaluation(nn.Module):
    def __init__(self, G=8, stage=3, evaluate_neighbors=9, iterations=2, double_weight=True):
    ...
    def forward(self, ref_feature, src_features, ref_proj, src_projs, depth_sample, depth_min, depth_max, iter,
        ...
        # similarity_sum = 0
        similarity_weight_sum = 1e-8
        similarities = 0
        if self.double_weight:
            attn_weight_sum = 1e-8
            attn_weights = 0
        ...
        if self.stage == 3 and view_weights == None:
            ...
            for src_idx, (src_feature, src_proj) in enumerate(zip(src_features, src_projs)):
                ...
                # ref_volume = ref_volume.reshape(batch, self.G, feature_channel // self.G, num_depth, height, width)
                # group-wise correlation
                # similarity = (warped_feature * ref_volume).mean(2)  # [B, G, D, H, W]
                similarity = (warped_feature * ref_feature.unsqueeze(3)).mean(2)

                # feature-wise view weight
                if self.double_weight:
                    attn_weight = torch.softmax(similarity.sum(1) / 2, 1) / math.sqrt(
                        feature_channel)  # [B, D, H, W]
                # pixel-wise view weight
                view_weight = self.pixel_wise_net(similarity)
                view_weights.append(view_weight)

                # Cross View Weight Aggregation
                if self.training:
                    similarity_weight_sum = similarity_weight_sum + view_weight
                    similarities = similarities + view_weight.unsqueeze(1) * similarity
                    if self.double_weight:
                        attn_weight_sum = attn_weight_sum + attn_weight
                        attn_weights = attn_weights + attn_weight.unsqueeze(1) * similarity
                        del attn_weight
                else:
                    similarity_weight_sum += view_weight
                    similarities += view_weight.unsqueeze(1) * similarity
                    if self.double_weight:
                        attn_weight_sum += attn_weight
                        attn_weights += attn_weight.unsqueeze(1) * similarity
                        del attn_weight
                del view_weight, similarity
            del src_features, src_projs

            view_weights = torch.cat(view_weights, dim=1)  # [B,4,H,W], 4 is the number of source views

            if self.double_weight:
                similarity1 = similarities.div_(similarity_weight_sum.unsqueeze(1))
                similarity2 = attn_weights.div_(attn_weight_sum.unsqueeze(1))
                similarity = similarity1 + similarity2
                del attn_weight_sum, similarity2, similarity1, attn_weights
            else:
                similarity = similarities.div_(similarity_weight_sum.unsqueeze(1))
            del similarity_weight_sum, similarities
            ...
            prob_volume = torch.exp(self.softmax(score))
            depth = depth_wta(prob_volume, depth_values=depth_sample)

            # depth regression: expectation
            # depth_sample = torch.sum(depth_sample * score, dim=1)

            return depth, prob_volume, view_weights.detach(), depth_sample
        else:
            i = 0
            for src_feature, src_proj in zip(src_features, src_projs):
                ...
                warped_feature = warped_feature.view(batch, self.G, feature_channel // self.G, num_depth, height, width)
                # ref_volume = ref_volume.reshape(batch, self.G, feature_channel // self.G, num_depth, height, width)
                # similarity = (warped_feature * ref_volume).mean(2)
                similarity = (warped_feature * ref_feature.unsqueeze(3)).mean(2)
                ...
                if self.training:
                    ...
                    similarities = similarities + view_weight.unsqueeze(1) * similarity
                    if self.double_weight:
                        attn_weight_sum = similarity_weight_sum + attn_weight
                        attn_weights = attn_weights + attn_weight.unsqueeze(1) * similarity
                        del attn_weight
                else:
                    similarity_weight_sum += view_weight
            ...
            # # depth regression: expectation
            # else:
            #     # attn_max_indices = score.max(1, keepdim=True)[1]  # B 1 H W
            #     # depth_sample = torch.gather(depth_sample, 1, attn_max_indices).squeeze(1)  # B H W
            #     depth_sample = torch.sum(depth_sample * score, dim=1)

            return depth, prob_volume, depth_sample
...
class PatchMatch(nn.Module):
    def __init__(self, random_initialization=False, propagation_out_range=2,
                 ...
                 num_feature=64, G=8, propagate_neighbors=16, stage=3, evaluate_neighbors=9, double_weight=True):
    ...
    def forward(self, ref_feature, src_features, ref_proj, src_projs, depth_min, depth_max,
        ...
        if self.random_initialization:
            ...
            depth_sample, prob_volume, view_weights, depth_value = self.evaluation(ref_feature, src_features, ref_proj,
                                                                                   src_projs,
                                                                                   depth_sample, depth_min, depth_max,
                                                                                   iter, eval_grid,
                                                                                   weight, view_weights)
            ...
            depth_sample, prob_volume, depth_value = self.evaluation(ref_feature, src_features, ref_proj, src_projs,
                                                                     depth_sample, depth_min, depth_max, iter,
                                                                     eval_grid, weight,
                                                                     view_weights)
            depth_sample = depth_sample.unsqueeze(1)
            depth_samples.append(depth_sample)
        ...
        for iter in range(2, self.patchmatch_iteration + 1):
            ...
            # evaluation, outputs regressed depth map
            depth, prob_volume, depth_value = self.evaluation(ref_feature, src_features,
                                                              ref_proj, src_projs, depth_sample, depth_min, depth_max,
                                                              iter,
                                                              eval_grid, weight, view_weights)

            depth_sample = depth.unsqueeze(1)
            depth_samples.append(depth_sample)
            prob_volumes.append(prob_volume)
            depth_values.append(depth_value)

        return depth_samples, prob_volumes, view_weights, depth_values
